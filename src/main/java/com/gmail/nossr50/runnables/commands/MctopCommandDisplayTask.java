...
public class MctopCommandDisplayTask extends BukkitRunnable {
    ...
    private void displayChat() {
        ...
        for (PlayerStat stat : userStats) {
            ...
            String statOutput = LocaleLoader.getString("Commands.Skill.LeaderboardEntry", String.format("%2d", place), stat.name, stat.statVal);

            if (sender instanceof Player) {
                sender.sendMessage(statOutput);
            } else {
                sender.sendMessage(ChatColor.stripColor(statOutput));
            }
