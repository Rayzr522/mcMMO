...
public class mcMMO extends JavaPlugin {
    ...
    private void registerCustomRecipes() {
        if (Config.getInstance().getChimaeraEnabled()) {
            ShapelessRecipe chimaeraWingRecipe = ChimaeraWing.getChimaeraWingRecipe();

            boolean alreadyRegistered = getServer().getRecipesFor(ChimaeraWing.getChimaeraWing(1)).stream()
                    .filter(recipe -> recipe instanceof ShapelessRecipe)
                    .map(recipe -> (ShapelessRecipe) recipe)
                    .anyMatch(recipe -> recipe.getKey().equals(chimaeraWingRecipe.getKey()));

            if (!alreadyRegistered) {
                getServer().addRecipe(chimaeraWingRecipe);
            }
        }
